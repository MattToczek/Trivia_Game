...
let createQAndAPairs = data => {
  ...
  data.results.forEach((element, num) => {
    ...
    }" class="inputBtns" name="question${num +
      1}" required><span class="inputText correct">${
      element.correct_answer
    }</span></input>`;
    ...
    answerArray.forEach((e, key) => {
      answerArray[
        ...
        1}" required><span class="inputText">${e}</span></input>`;
    });
  ...
  if (req.session.loggedin) {
    db.query("SELECT * FROM topScores", function(error, results, fields) {
      ...
      if (sortedScores.length > 5) {
        sortedScores = sortedScores.splice(
...
app.post("/scoreRead", (req, res) => {
  ...
  if (req.session.loggedin) {
    ...
    // WORKING SQL Query
    let writeHs =
      "INSERT INTO high_scores SET user_id = ?, user_name = ?, user_score = ?";
...
app.post("/sucessfulSignUp", (request, response) => {
  ...
  let scoreBoardAdd =
    "INSERT INTO high_scores SET user_name = ?, user_id = ?, user_score = ?";
